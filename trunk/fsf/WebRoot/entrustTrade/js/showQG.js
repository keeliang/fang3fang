...
function ShowDetailQG(){
	...
	$.ajax({
		...
		type: "POST",
		dataType: "xml",
		data: "useraction=Estates&referaction=SelectEstateByID&SellorRent=0&RequireSellRent=1&IsConsign=1&EstateMsgID=" + id,
		cache: false,
		success: function(xml){
				...
				var StartPrice=$(properties).find("ManagementFees").text();
				var EndPrice=$(properties).find("Cost").text();
				$("#StartPrice").empty();
				$("#StartPrice").append('<span class="cOrange">'+StartPrice+'</span>  万元');
				$("#EndPrice").empty();
				$("#EndPrice").append('<span class="cOrange">'+EndPrice+'</span>  万元'); 
