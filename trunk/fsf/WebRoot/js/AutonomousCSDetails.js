...
$(document).ready(function() {
	...
	$.ajax({
		...
		type: "GET",
		dataType: "xml",
		data: "referaction=SelectAutonomousCSByACSID&ACSID=" + window.id,
		success: function(xml){
			var properties = $(xml).find("AutonomousCS")[0];
			$(properties).children().each(function() {
				$(this).text($.trim($(this).text()));
			})
			
			var ACSID = $(properties).find("ACSID").text();
			
			$("#ACSID").val(ACSID);
			$("#ShowCSID").append(ACSID);
