$(document).ready(function() {
	...
	if (title != -1) {
		...
		if(titleType == 'D') {
			...
			var dataValue = "referaction=SelectAutonomousCSByDistrictName&DistrictName=" + title;
		} else if (titleType == 'T') {
			//T是商圈
			url = "/Fang3FangWeb/servlet/AllFangPanByTownName";
			var dataValue = "referaction=SelectAutonomousCSByTownName&TownName=" + title;
		} else if (titleType == 'C') {
			//C是价格
			...
			var dataValue = "referaction=SelectAutonomousCSByCost&Cost1=" + Cost1 + "&Cost2=" + Cost2;
			title = title + "&nbsp;万元";
		} else if (titleType == 'R') {
			//R是多少间房
			url = "/Fang3FangWeb/servlet/AllFangPanByRoom";
			var dataValue = "referaction=SelectAutonomousCSByRoom&Room=" + title;
			title = title + "室";
		}
	} else {
		//显示所有信息
		url = "/Fang3FangWeb/servlet/SearchAutonomousCS";
		var dataValue = "referaction=SelectAutonomousCS";
	}
	...
	$.ajax({
		...
		success: function(xml){
			...
			if(AutonomousCS.length > 0) {				
				for(var i = 0; i < AutonomousCS.length; i++) {
