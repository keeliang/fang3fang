...
$(document).ready(function() {
	...
	$.ajax({
		...
		type: "GET",
		dataType: "xml",
		data: "referaction=SelectProxyQZByPQZID&PQZID=" + window.id,
		success: function(xml){
			var properties = $(xml).find("ProxyQZ")[0];
			$(properties).children().each(function() {
				$(this).text($.trim($(this).text()));
			})
			
			var PQZID = $(properties).find("PQZID").text();
			
			$("#PQZID").val(PQZID);
			$("#ShowQZID").append(PQZID);
			...
			var DepositWay = $(properties).find("DepositWay").text();
				ChangeSelectedObj("DepositID", DepositWay);
