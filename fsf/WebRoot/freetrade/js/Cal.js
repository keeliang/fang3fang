...
lilv_array[37][1][5]=0.0426;//商贷 1～5年 4.91%
lilv_array[37][1][10]=0.0428;//商贷 5-30年 5.04%
lilv_array[37][2][5]=0.0351;//公积金 1～5年 4.05%
lilv_array[37][2][10]=0.0405;//公积金 5-30年 4.59%

//2008年	11.27日新利率上限
lilv_array[38]=new Array;
lilv_array[38][1]=new Array;
lilv_array[38][2]=new Array;
lilv_array[38][1][5]=0.0653;//商贷 1～5年 7.72%
lilv_array[38][1][10]=0.0673;//商贷 5-30年 7.92%
lilv_array[38][2][5]=0.0351;//公积金 1～5年 4.05%
lilv_array[38][2][10]=0.0405;//公积金 5-30年 4.59%

//2008年	11.27日新利率基准
lilv_array[39]=new Array;
lilv_array[39][1]=new Array;
lilv_array[39][2]=new Array;
lilv_array[39][1][5]=0.0594;//商贷 1～5年 7.02%
lilv_array[39][1][10]=0.0612;//商贷 5-30年 7.20%
lilv_array[39][2][5]=0.0351;//公积金 1～5年 4.05%
lilv_array[39][2][10]=0.0405;//公积金 5-30年 4.59%

//2008年	11.27日新利率(第二套房)
lilv_array[40]=new Array;
lilv_array[40][1]=new Array;
lilv_array[40][2]=new Array;
lilv_array[40][1][5]=0.0653;//商贷 1～5年 7.72%
lilv_array[40][1][10]=0.0673;//商贷 5-30年 7.92%
lilv_array[40][2][5]=0.0351;//公积金 1～5年 4.05%
lilv_array[40][2][10]=0.0405;//公积金 5-30年 4.59%
...
lilv_array[41][1][5]=0.04032;//商贷 1～5年 4.032%
lilv_array[41][1][10]=0.04158;//商贷 5-30年 4.158%
...
lilv_array[42][1][5]=0.0576;//商贷 1～5年 5.76%
lilv_array[42][1][10]=0.0594;//商贷 5-30年 5.94%
	...
	if (fmobj.type.value == 3 ){
		//--  组合型贷款(组合型贷款的计算，只和商业贷款额、和公积金贷款额有关，和按贷款总额计算无关)
			...
			//1.本金还款
				...
				fmobj.month_money.value = month_money;
				//还款总额
				fmobj.all_total.value = Math.round(all_total*100)/100;
				//支付利息款
				fmobj.accrual.value = Math.round( (all_total - daikuan_total) *100)/100;


			//2.本息还款
				//月均还款
				var month_money = getMonthMoney(lilv_sd,total_sy,month) + getMonthMoney(lilv_gjj,total_gjj,month);//调用函数计算
				fmobj.month_money.value = Math.round(month_money*100)/100 + "(元)";
				//还款总额
		...
		//--  商业贷款、公积金贷款
			...
			if (fmobj.jisuan_radio[0].checked == true){
				...
				//房款总额
				var fangkuan_tota = fmobj.price.value * fmobj.sqm.value;
			...
			//1.本金还款
				...
				for(j=0;j<month;j++) {
					//调用函数计算: 本金月还款额
					huankuan = getMonthMoney(lilv,daikuan_total,month,j);
					all_total += huankuan;
					huankuan = Math.round(huankuan*100)/100;
					//fmobj.month_money2.options[j] = new Option( (j+1) +"月," + huankuan + "(元)", huankuan);
					month_money += (j+1) +"月," + huankuan + "(元)\n";
			...
			//2.本息还款
				//月均还款
				var month_money = getMonthMoney1(lilv,daikuan_total,month);//调用函数计算
				fmobj.month_money.value = Math.round(month_money1*100)/100 + "(元)";
				//还款总额
				var all_total = month_money1 * month;
				fmobj.all_total.value = Math.round(all_total*100)/100;
				//支付利息款
				fmobj.accrual1.value = Math.round( (all_total - daikuan_total) *100)/100;
...
//税费计算器
function runjs3(obj){dj3=parseFloat(obj.dj3.value)
...
function SetIFameHeight(obj1name,obj2name)
{
    parent.document.getElementById(obj1name).height=document.getElementById(obj2name).offsetHeight;
}