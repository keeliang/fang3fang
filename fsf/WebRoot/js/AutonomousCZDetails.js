...
$(document).ready(function() {
	...
	$.ajax({
		...
		type: "GET",
		dataType: "xml",
		data: "referaction=SelectAutonomousCZByACZID&ACZID=" + window.id,
		success: function(xml){
			var properties = $(xml).find("AutonomousCZ")[0];
			$(properties).children().each(function() {
				$(this).text($.trim($(this).text()));
			})
			
			var ACZID = $(properties).find("ACZID").text();
			
			$("#ACZID").val(ACZID);
			$("#ShowCZID").append(ACZID);
