...
function UpdateQZ(){
	...
	$.ajax({
		...
		type: "POST",
		dataType: "xml",
		data: "useraction=Estates&referaction=getUpdateEstateByID&SellorRent=1&RequireSellRent=1&IsConsign=0&EstateMsgID=" + id,
		cache: false,
		success: function(xml){
			   ...
			   $("#PropertyName").val(PropertyName);		    
				...
				var DepositID = $(properties).find("DepositID").text();                         //押金类型
		...
		//		alert(QQ);
		        ...
		        */
				...
				var DepositID=$(properties).find("DepositID").text();                         //押金要求
  ...
  $('#QZForm').ajaxForm({
	//	beforeSubmit: validate,	/*提交前先控制*/		 
		success: function() {
	        ...
	        window.location = "ListAutonomousQZ.jsp";
		},
		error: function() {
