    ...
    $('#CancelForm').ajaxForm({		 
		...
		success: function(xml) {			  
			...
			for(var i = 0; i < FileName.length; i++) {
			    var imageID = $.trim(FileName[i].getElementsByTagName("imageID")[0].firstChild.nodeValue);	    			  
                var filename = $.trim(FileName[i].getElementsByTagName("filename1")[0].firstChild.nodeValue);	       
	            $("#filename").append("<option value = '" + imageID + "'>" + filename + "</option>") 
			}			   
		},
		error: function() {			 
...
function validate() { 
	if (!(
	  ...
	  return false;
	}else if (!($("#filename").children("option").size()< 5)) {
	  alert("只允许粘贴五个图片文件");
	  return false;
	}
}
