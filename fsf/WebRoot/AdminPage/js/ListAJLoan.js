...
alert("按揭管理");
	...
	$.ajax({
		...
		type: "POST",
		dataType: "xml",
		data: "useraction=Estates&referaction=SelectApplyForLoansInfoAdmin",
			...
			if($(properties).length > 0) {
				for(var i = 0; i < $(properties).length; i++) {
					...
					$("#ListAJLoanTable tbody").append('<tr><td><a href="UpdateAJLoan.jsp?AjID='+AjID+'" title="'+AjID+'">'+AjID+'</a></TD><TD>'+RealName+'</TD>' +
							...
							'<TD>&nbsp;</TD><TD><span class="float_right"><a href="/Fang3FangWeb/Access?AjID='+AjID+'&useraction=Estates&referaction=DeleteAJLoanInfoAdmin" class="cRed">【删除】</a></span></TD></TR>');					 
